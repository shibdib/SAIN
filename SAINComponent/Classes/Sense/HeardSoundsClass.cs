...
namespace SAIN.SAINComponent.Classes.Sense
{
    public class HeardSoundsClass : SAINBase, ISAINClass
    {
        ...
        private void updateCache()
        {
            if (_nextUpdateCacheTime < Time.time)
            {
                ...
                if (highestPower != null)
                {
                    ...
                    {
                        if (SAIN.Hearing.EnemySoundHeard(
                    ...
                    else if (SAIN.Hearing.EnemySoundHeard(
                            highestPower.SourcePlayer,
                            highestPower.Position,
                            highestPower.Power,
                            AISoundType.step))
        ...
        public void AddSound(SAINSoundType soundType, Vector3 position, IPlayer sourcePlayer, float power)
        {
            ...
            if (sourcePlayer == null || sourcePlayer.Transform == null || sourcePlayer.ProfileId == SAIN.ProfileId)
            {
                return;
            }
            if ((position - SAIN.Position).sqrMagnitude > power * power)
            {
                return;
            }
            if (SAIN.Hearing.IsPlayerFriendly(sourcePlayer))
            {
                return;
            }
